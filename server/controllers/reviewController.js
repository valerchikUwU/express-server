...
exports.ordersReview_create_post = [
  ...
  body(
    ...
    "organizationCustomerIds должен быть массивом идентификаторов"
  )
    .custom((ids) => {
      ...
      if (ids.length === 0) {
        throw new Error(
          "organizationCustomerIds должен содержать хотя бы одно значение"
        );
      }
      return true;
    })
    .escape(),
  body("status", "Статус должен быть указан").isInt({ min: 1 }).escape(),
        ...
        const order = await Order.findAll({
          where: {
            ...
            organizationCustomerId: {
              [Op.in]: [req.body.organizationCustomerId],
            },
            payeeId: {
              [Op.in]: [req.body.payeeId],
            },
          },
          attributes: {
            include: [
        ...
        const order = await Order.findAll({
          ...
          attributes: {
            include: [
              [
                Sequelize.literal(
                  `SUM(CASE WHEN productTypeId <> 4 THEN (CASE WHEN addBooklet = TRUE THEN quantity * priceBooklet ELSE quantity * priceAccess END) END)`
                ),
                "SUM",
              ],
            ],
